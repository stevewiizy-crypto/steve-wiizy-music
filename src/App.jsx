    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-md mx-auto p-4">
        <div className="text-center">
          <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-yellow-400 to-red-600 flex items-center justify-center text-2xl font-black -mt-2">SW</div>
          <h1 className="text-4xl font-black text-red-500 mt-2 tracking-tight">STEVE WIIZY</h1>
        ...
        {/* SEARCH - NEW */}
        <div className="mt-8">
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Search songs... love, gospel, vibe..." className="w-full p-4 rounded-full bg-zinc-900 border border-zinc-800 text-white placeholder-zinc-500 outline-none focus:border-red-600" />
        </div>

        <h3 className="text-xl font-bold mt-8">10 Latest Tracks 🔥 - Free Download</h3>

        <div className="flex flex-wrap gap-2 justify-center mt-4">
          {CATS.map(c=><button key={c} onClick={()=>setCat(c)} className={`px-4 py-2 rounded-full text-sm border ${cat===c?'bg-red-600 border-red-600 text-white':'bg-zinc-900 border-zinc-800 text-white'}`}>{c}</button>)}
        </div>
